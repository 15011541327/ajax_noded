...
window.onload = function () {
	...
	document.getElementById('asyncBtn').onclick = function () {
		...
		//3. 打开一个连接
		request.open('POST', '/node_ajax/test/async_sync2'); //异步ajax
		//4. 设置请求头, 对请求体数据进行编码处理
		request.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		//5. 发请求
		request.send('username=Bob');
		alert('after send() ');
	};
	...
	document.getElementById('syncBtn').onclick = function () {
		...
		//3. 打开一个连接
		request.open('POST', '/node_ajax/test/async_sync2', false); //同步Ajax
		//4. 设置请求头, 对请求体数据进行编码处理
		request.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		//5. 发请求
		request.send('username=Bob2');
		alert('after send() ');
	};
